...
function preload() {
  cockroachSpritesheet = loadImage("media/cockroach.png"); // Walking animation sprite sheet
  squishedCockroach = loadImage("media/bug_squished.png"); // Squished bug image
} 

function setup() { 
  createCanvas(400, 400); 

  // Create multiple cockroaches
  for (let i = 0; i < 5; i++) {
...
function increaseBugSpeed() {
  // Make remaining bugs move faster after each squish
  for (let bug of bugs) {
    bug.speed *= 1.1;
  }
}
...
class Character {
  ...
  isClicked(mx, my) {
    return dist(mx, my, this.x, this.y) < 25; // Approximate size of the bug
  }
  ...
  squish() {
    ...
    setTimeout(() => {
      ...
      if (index > -1) {
        bugs.splice(index, 1); // Remove squished bug
      }
    }, 200); // Delay for squished effect
  }
}
